...
(function () {
    ...
    function Signaler(root) {
        ...
        if (!root.openSignalingChannel) {
            ...
            socket.on('child_added', function (snap) {
                ...
                if (data.userid != userid) {
                    if (data.leaving && root.onuserleft) root.onuserleft(data.userid);else signaler.onmessage(data);
                }
            ...
            var socket = root.openSignalingChannel(function (message) {
                ...
                if (message.userid != userid) {
                    if (message.leaving && root.onuserleft) root.onuserleft(message.userid);else signaler.onmessage(message);
                }
            });
        ...
        this.onmessage = function (message) {
            ...
            if (message.roomid && message.broadcasting && !signaler.sentParticipationRequest) root.onmeeting(message);else
                // for pretty logging
        ...
        this.onice = function (message) {
            ...
            if (!peer) {
                ...
                if (candidate) candidates[message.userid][candidate.length] = message.candidate;else candidates[message.userid] = [message.candidate];
            } else {
        ...
        var options = {
            onsdp: function onsdp(sdp, to) {
                signaler.signal({
                    sdp: sdp,
                    to: to
                });
            },
            onicecandidate: function onicecandidate(candidate, to) {
                signaler.signal({
                    candidate: candidate,
                    to: to
                });
            },
            onaddstream: function onaddstream(stream, _userid) {
    ...
    var Offer = {
        createOffer: function createOffer(config) {
            ...
            if (config.onaddstream) peer.onaddstream = function (event) {
                config.onaddstream(event.stream, config.to);
            };
            if (config.onicecandidate) peer.onicecandidate = function (event) {
        ...
        setRemoteDescription: function setRemoteDescription(sdp) {
