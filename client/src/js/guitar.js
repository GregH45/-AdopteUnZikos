...
document.addEventListener('keydown', (event) => {
  /* Plus besoin, gestion du stop par le timer
  // Gestion pour arreter animation de playguitar
  var GratteEnCours = false; // false = pas de gratte
  for (var i = 0; i < historique_gratte.length; i++) {
    if(historique_gratte[i]){
      GratteEnCours = true; // Si un true dans tableau alors une gratte est en cours -> GratteEnCours = true
      break;
    }
  }
  if (!(GratteEnCours)) {
    var ObjetStopGuitar = new Object();
    ObjetStopGuitar.type = "play";
    ObjetStopGuitar.stop = true;
    App.Messenger.sendMessage('play', ObjetStopGuitar);
  }
  */
  ...
  if (whoCase(maGuitar, nomTouche) != -1) {
    ...
    if(historique_gratte[numero_corde_case] == true){
      ...
      ObjectOneSon.type = "play";
      App.Messenger.sendMessage('play', ObjectOneSon);
